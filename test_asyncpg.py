...
from asyncdb.providers.pg import pgPool, pg
...
async def cursor(d):
    async for record in await d.cursor("SELECT store_id, store_name FROM walmart.stores"):
        print(record)
...
with sa.connection() as conn:
    ...
    result, error = newloop.run_until_complete(conn.query(users.select(users.c.name.startswith('D'))))
    print(result)
    # Drop the table
