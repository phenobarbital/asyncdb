...
async def connect(c):
    async with await c.connection() as conn:
        ...
        if error:
            print(error)
        # get a query
        stores, error = await conn.query("SELECT store_id, store_name FROM walmart.stores")
        #print(result)
        # execute a sentence
        ...
        error = await conn.executemany('INSERT INTO test.stores (store_id, store_name) VALUES ($1, $2)', *st)
        print("DELETING ROWS")
        ...
        # iterate a cursor:
        async for record in await c.cursor("SELECT store_id, store_name FROM walmart.stores"):
            print(record)
        ...
        async with await c.transaction() as t:
            ...
            await t.commit()
        # table copy
        await c.copy_from_table(table = 'stores', schema = 'walmart', columns = [ 'store_id', 'store_name'], output = 'stores.csv')
