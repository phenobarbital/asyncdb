...
class rethink(BaseProvider):
    ...
    async def connection(self):
        logger.debug("RT Connection to host {} on port {} to database {}".format(self._params['host'], self._params['port'], self._params['db']))
        try:
    ...
    async def query(self, table, filter=None):
        ...
        if self._connection:
            ...
            try:
                ...
                self._columns = await self._engine.table(table).nth(0).default(None).keys().run(conn)
                print(self._columns)
                if not filter:
    ...
    async def queryrow(self, table, filter = {}, id = 0):
        ...
        if self._connection:
            ...
            try:
                data = await self._engine.table(table).filter(filter).nth(id).run(self._connection)
                if data:
                    self._result = data
                else:
                    raise NoDataFound(message = "Empty Result", code = 404)
    ...
    async def get_all(self, table, filter = [], index = ''):
        ...
        if self._connection:
            try:
                if index:
                    cursor = await self._engine.table(table).get_all(filter, index = index).run(self._connection)
                else:
                    cursor = await self._engine.table(table).get_all(filter).run(self._connection)
                data = []
                while (await cursor.fetch_next()):
    ...
    async def match(self, table, field = 'id', regexp = '(?i)^[a-z]+$'):
        ...
        if self._connection:
            try:
                data = await self._engine.table(table).filter(lambda doc:
                    doc[field].match(regexp)
    ...
    async def replace(self, table, id, data):
        ...
        if self._connection:
            try:
                self._result = await self._engine.table(table).get(id).replace(data).run(self._connection)
                if self._result['errors'] > 0:
                    raise ProviderError("REPLACE Runtime Error: {}".format(self._result['first_error']))
                    return False
                return self._result
            except RqlRuntimeError as err:
    ...
    async def update(self, table, data, id=None, filter = {}):
        ...
        if self._connection:
            if id:
                ...
                return await self._engine.table(table).update(data).run(self._connection)
        else:
            return False
    ...
    async def literal(self, table, id, data):
        ...
        if self._connection:
            try:
                self._result = await self._engine.table(table).get(id).update({ field: r.literal(data).run(self._connection)})
                return self._result
            except RqlRuntimeError as err:
    ...
    async def update_conditions(self, table, data, filter={}, fieldname='filterdate'):
        ...
        if self._connection:
            try:
                self._result = await self._engine.table(table).filter(~self._engine.row.has_fields(fieldname)).filter(filter).update(data).run(self._connection)
                return self._result
            except RqlRuntimeError as err:
    ...
    async def delete(self, table, id=None, filter = {}, changes = True):
        ...
        if self._connection:
            if id:
                try:
                    self._result = await self._engine.table(table).get(id).delete(return_changes=changes).run(self._connection)
                    return self._result
                except (RqlRuntimeError, ReqlRuntimeError, ReqlError) as err:
                    raise ProviderError(err)
                    return False
                ...
                try:
                    self._result = await self._engine.table(table).filter(filter).delete(return_changes=changes).run(self._connection)
                    return self._result
                except (RqlRuntimeError, ReqlRuntimeError, ReqlError) as err:
                    raise ProviderError(err)
                    return False
                ...
                return await self._engine.table(table).delete(return_changes=changes).run(self._connection)
        else:
            return False
        ...
        if conditions and len(conditions) > 0:
            for key, value in conditions.items():
                ...
                if type(value) == list:
                    ...
                    elif self.cond_definition and self.cond_definition[key] == 'boolean':
                        self.conditions[key] = value
    ...
    def query_options(self, result, conditions):
        print("PROGRAM FOR QUERY IS {} for option {}".format(self._program, self.qry_options))
        if self.qry_options:
            hierarchy = get_hierarchy(self._program)
            if hierarchy:
                try:
                    get_filter = [ k.replace('!', '') for k in conditions if k.replace('!', '') in hierarchy]
    ...
    async def get_one(self, table, filter = {}, id = 0):
        ...
        if self._connection:
            try:
                self._result = await self._engine.table(table).filter(filter).nth(id).run(self._connection)
                return self._result
            except ReqlNonExistenceError as err:
    ...
    async def run(self, table):
        ...
        for key, value in conditions.items():
            if type(value) is list:
                print(value)
                search = search.filter((lambda exp: self._engine.expr(value).coerce_to('array').contains(exp[key])))
            else:
                if type(value) is str:
                    if value.startswith("!"):
                        # not null
                        result = result.and_(self._engine.row[key].ne(value.replace('!', '')))
